def norm_circle(points, width_cells, height_cells, fps, padding=0.07, isYminus=False):
...
def norm1(points, width_cells, height_cells, h, padding=0.1):
    xs = [x for _, x, _ in points]
    ...
    xmin, xmax = min(xs), max(xs)
    if xmax <= 0:
        points = [(f, x + abs(xmin), y) for f, x, y in points]
        xs = [x for _, x, _ in points]
        xmin, xmax = min(xs), max(xs)
    ...
    norm_points = []
    for f, x, y in points:
        px = (x - xmin) / scale * (Wpx - 1)
        py = (y - ymin) / scale * (Hpx - 1) * 0.8 * (1.5 + 0.5/max(h, 0.5))
        norm_points.append((f, px, py))
    return norm_points, Wpx, Hpx
